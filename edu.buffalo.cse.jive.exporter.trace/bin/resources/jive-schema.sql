...
CREATE TABLE event_kind AS (
  ...
  (VALUES
    ...
    ( 6,'METHOD_CALL'),             --  7 --> 6
    ( 7,'METHOD_ENTERED'),          --  8 --> 7
    ( 8,'METHOD_EXIT'),             --  9 --> 8
    ( 9,'METHOD_RETURNED'),         -- 10 --> 9

    (10,'MONITOR_LOCK_BEGIN'),      -- 11 --> 10, thread_state --> TS_BLOCKED
    (11,'MONITOR_LOCK_END'),        -- 12 --> 11, thread_state --> TS_RUNNING
    (12,'MONITOR_LOCK_FAST'),       -- 13 --> 12, thread_state --> TS_BLOCKED
    (13,'MONITOR_RELOCK'),          -- 14 --> 13, thread_state --> TS_BLOCKED
    (14,'MONITOR_UNLOCK_BEGIN'),    -- 15 --> 14, thread_state --> unchanged
    (15,'MONITOR_UNLOCK_COMPLETE'), -- 16 --> 15, thread_state --> TS_RUNNING
    (16,'MONITOR_UNLOCK_END'),      -- 17 --> 16, thread_state --> TS_RUNNING if lockCount = 0
    (17,'MONITOR_UNLOCK_FAST'),     -- 18 --> 17, thread_state --> TS_RUNNING if lockCount = 0
    ...
    (20, 'SCOPE_ALLOC'),
    (21, 'SCOPE_ASSIGN'),
    (22, 'SCOPE_BACKING_ALLOC'),
    (23, 'SCOPE_BACKING_FREE'),
    (24, 'SCOPE_ENTER'),
    (25, 'SCOPE_EXIT'),
    (26, 'SCOPE_FREE'),
    (27, 'SCOPE_POP'),
    ...
    (29,'SYSTEM_END'),              -- 20 --> 29
    (30,'SYSTEM_START'),            -- 21 --> 30
    
    (31,'THREAD_CREATE'),           -- 22 --> 31, thread_state --> TS_CREATED
    (32,'THREAD_END'),              -- 23 --> 32, thread_state --> TS_TERMINATED
    (33,'THREAD_LOCK'),             --  6 --> 33, 
    (34,'THREAD_PRIORITY'),         -- 24 --> 34, thread_state --> unchanged
    (35,'THREAD_SLEEP'),            -- 25 --> 35, thread_state --> TS_WAITING
    (36,'THREAD_START'),            -- 26 --> 36, thread_state --> TS_RUNNING
    (37,'THREAD_WAKE'),             -- 27 --> 37, thread_state --> TS_RUNNING
    (38,'THREAD_YIELD'),            -- 28 --> 38, thread_state --> TS_WAITING
...
CREATE UNLOGGED TABLE contour (
  ...
  oid bigint,                 -- unique object identifier (for all contours that make up the object)
...
CREATE OR REPLACE FUNCTION tg_contour_after_update() RETURNS trigger AS $$
BEGIN
  UPDATE contour_member SET toTime = NEW.toTime WHERE contourId = NEW.contourId;
  -- return the unmodified tuple
  RETURN NEW;
END; $$ LANGUAGE 'plpgsql';
COMMENT ON FUNCTION tg_contour_after_update() IS 'Updates the end times of all contour members.';

CREATE TRIGGER tg_contour_after_update
  AFTER INSERT ON contour 
  FOR EACH ROW 
  EXECUTE PROCEDURE tg_contour_after_update();
COMMENT ON TRIGGER tg_contour_after_update ON contour IS 'Updates the end times of all contour members.';
...
CREATE UNLOGGED TABLE event_rtbackingalloc (
  eventId bigint,             -- references event.eventId
  size bigint                 -- size of the scope backing
);
COMMENT ON TABLE event_rtbackingalloc IS 'Mapping from IScopeBackingAllocEvent. A real-time event identifying a backing allocation.';

CREATE UNLOGGED TABLE event_rtscope (
  eventId bigint,             -- references event.eventId
  scope text                  -- scope on which the event occurred
);
COMMENT ON TABLE event_rtscope IS 'Mapping from IScopeEvent. A real-time event referencing a scope.';

CREATE UNLOGGED TABLE event_rtscopealloc (
  eventId bigint,             -- references event.eventId
  size bigint,                -- scope size
  immortal boolean            -- immortal scope
);
COMMENT ON TABLE event_rtscopealloc IS 'Mapping from IScopeAllocEvent. A real-time event identifying a scope allocation.';

CREATE UNLOGGED TABLE event_rtscopeassign (
  eventId bigint,             -- references event.eventId
  scopeLHS text,              -- scope of the object on the LHS
  indexLHS int,               -- stack index of the scope where the object on the LHS is allocated
  lhs bigint,                 -- value of the LHS object
  scopeRHS text,              -- scope of the object on the RHS
  indexRHS int,               -- stack index of the scope where the object on the RHS is allocated
  rhs bigint                  -- value of the RHS object
);
COMMENT ON TABLE event_rtscopeassign IS 'Mapping from IScopeAssignEvent. A real-time event identifying an assignment between objects, O1.F = O2 or O1[i] = O2.';
...
BEGIN
  ...
  */
  FOR ev IN SELECT eventId, threadId, kind, monitor 
            ...
            WHERE kind IN (10, 11, 12, 13, 15, 16, 17, 31, 32, 34, 35, 36, 37, 38)
            ORDER BY eventId 
  LOOP
    ...
    /*
      Monitor Events
      ----------------
      (10,'MONITOR_LOCK_BEGIN'),      -- 
      (11,'MONITOR_LOCK_END'),        -- holder --> event's thread (+1)
      (12,'MONITOR_LOCK_FAST'),       -- holder --> event's thread (+1)
      (13,'MONITOR_RELOCK'),          -- holder --> event's thread (set to prior)
      (14,'MONITOR_UNLOCK_BEGIN'),    -- 
      (15,'MONITOR_UNLOCK_COMPLETE'), -- holder --> event's thread (set to zero)
      (16,'MONITOR_UNLOCK_END'),      -- holder --> event's thread (-1)
      (17,'MONITOR_UNLOCK_FAST'),     -- holder --> event's thread (-1)
    */
    IF (ev.kind IN (11, 12, 13, 15, 16, 17)) THEN
      ...
      VALUES(
        ...
        CASE 
          ...
          WHEN ev.kind IN (16, 17) AND GREATEST(0, mon.lockCount - 1) = 0 THEN NULL
          -- otherwise, keep track of the holder
          ELSE ev.threadId
        END,
        CASE 
          -- simple increment
          WHEN ev.kind IN (11, 12) THEN COALESCE(mon.lockCount, 0) + 1
          -- simple decrement
          WHEN ev.kind IN (16, 17) THEN GREATEST(0, mon.lockCount - 1)
          -- decrement to zero
          WHEN ev.kind = 15 THEN 0
          ...
          ELSE (SELECT lockCount FROM monitor mi 
                WHERE toTime = (SELECT MAX(fromTime) FROM monitor mi INNER JOIN event e ON e.eventId = mi.fromTime
                                WHERE e.kind = 15 AND mi.monitor = ev.monitor AND mi.holder = mon.holder))
        END,
        ev.eventId);
    ...
    */
    ...
    ELSIF (ev.kind IN (10, 11, 12, 13, 32, 35, 36, 37, 38)) THEN
      ...
      SELECT INTO new_state 
        CASE 
          -- TS_RUNNING
          WHEN e.kind IN (11, 12, 13, 36, 37) THEN 2
          -- TS_BLOCKED
          WHEN e.kind = 10 THEN 3
          -- TS_WAITING
          WHEN e.kind IN (35, 38) THEN 4
          -- TS_TERMINATED
          ELSE 5
        END
      FROM event e
