...
public final class ValueFactory_MM implements IValueFactory
{
  ...
  ValueFactory_MM(final ExecutionModel model)
  {
    this.model = model;
    VALUE_NULL = new NullValue();
    VALUE_SYSTEM = new SystemCallerValue();
    VALUE_UNINITIALIZED = new UninitializedValue();
    SYSTEM_THREAD = createThread(ValueFactory_MM.SYSTEM_THREAD_ID, "SYSTEM");
  ...
  // LAST UPDATE: replaced string with a reference
  private final class FileValue extends AbstractValue implements IFileValue
  {
    private final IReference refFileName;

    private FileValue(final String fileName)
    {
      this.refFileName = model.memoryStore().putString(fileName);
    }
    ...
    public String name()
    {
      return model.memoryStore().getString(refFileName);
    }
  ...
  {
    private final IReference refMethodKey;

    OutOfModelMethodKeyReference(final String description, final String methodKey)
    {
      super(description);
      this.refMethodKey = model.memoryStore().putString(methodKey);
    }

    @Override
    public String key()
    {
      return model.memoryStore().getString(refMethodKey);
    }
  ...
  {
    private final IReference refTypeName;

    private ResolvedValue(final String value, final String typeName)
    {
      super(value);
      this.refTypeName = model.memoryStore().putString(typeName);
    }
    ...
    {
      return model.memoryStore().getString(refTypeName);
    }
  }
  ...
  private abstract class Value extends AbstractValue implements IValue
  {
    ...
    public String value()
    {
      return model.memoryStore().getString(refStringValue);
    }
