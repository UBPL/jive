...
class Disjunction extends StructuredExpression implements IDisjunction
{
  private boolean isLiteral;
  private final List<IExpression> members = new ArrayList<IExpression>();
  private Type type = null;

  Disjunction(final IExpression expression) throws ExpressionException
  {
  ...
  {
    if (visitor.visitDisjunction(this, arg))
    {
  ...
  @Override
  public IDisjunction append(final IExpression expression) throws ExpressionException
  {
    // catch null expression
    if (expression == null)
    {
    ...
    final Type t = TypeUtils.inferForDisjunction(type, expression.getType());
    // catch invalid type
    if (t == Type.INVALID)
    {
      dieInvalid(expression);
  ...
  public String toString()
  {
    ...
    {
      result.append(" OR " + members.get(i).toString());
    }
  ...
  public String toStringTyped()
  {
    ...
    for (int i = 1; i < members.size(); i++)
    {
      result.append(" OR " + members.get(i).toStringTyped());
    }
  ...
  private void dieInvalid(final IExpression expression) throws ExpressionException
  {
    throw new ExpressionException(String.format(
        "Error appending disjunct '%s' to expression '%s'.", expression == null ? "null"
